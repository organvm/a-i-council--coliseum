...
class EventPrioritizer:
    ...
    def calculate_score(self, event: NormalizedEvent, now: datetime = None) -> float:
        """
        ...
        # Optimization: Reuse passed 'now' to avoid N system calls
        if now is None:
            now = datetime.utcnow()
    ...
    def prioritize_events(self, events: List[NormalizedEvent]) -> List[NormalizedEvent]:
        ...
        # Optimization: Calculate 'now' once for the entire batch
        now = datetime.utcnow()
