...
class Agent(BaseAgent):
    ...
    async def process_message(self, message: Message) -> Optional[Message]:
        ...
        # 2. Analyze content using NLP (Restored Feature)
        ...
        if not should_respond:
            ...
            # Or if active (logic from previous version)
            if self.state.is_active and message.recipient_id is None:
                 # In a real system, we'd use a probability or relevance score
                 # For now, let's keep it minimal to avoid noise
                 pass
